...
% gets all parameters from "analysisInput.m"
run('C:\Users\Simon\Dropbox (Personal)\Research\PhD\research-projects\kics-project\simulation-compilers\local\analysisInput.m') 
...
if createSim
    ...
    [J,true_params] = kicsSim(sz,T,w0,N_diff,D,k_on,k_off,k_p,...
        mean_agg_num,std_agg_dist,num_filaments,prob_place,'parallel',...
        sim_parallel,'nParts',n_parts,'subFrames',n_sub_frames,...
        'savepath',save_simfile,'noiseType',noise_type,'kerVar',...
        kernel_varargin,'laserVar',laser_varargin,'noiseVar',...
        noise_varargin,'fracDiff',frac_diff,'mask',mask_struct);
...
% err = @(params) kICSSlideWinFit(params,kSqVectorSubset,tauVector,k_p_fit,size(J,3),201,...
%     'err',r_k_norm,'symvars',{''});
% fit_fun = @(params,ksq,tau) kICSSlideWinFit(params,ksq,tau,k_p,size(J,3),201,'symvars',{''});
