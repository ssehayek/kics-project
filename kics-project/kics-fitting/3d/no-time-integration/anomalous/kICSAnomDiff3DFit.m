% kICSAnomDiff3DFit(...) help header

function out = kICSAnomDiff3DFit(params,kSq,tauVector,varargin)
...
s = struct('D',params(1),'r',params(2),'K',params(3),'frac',params(4),'z0',params(5),'alpha',params(6));

[tauGrid,kSqGrid] = meshgrid(tauVector,kSq);

cxn_3d = 1./sqrt(4.*s.D.*tauGrid.^s.alpha+s.z0.^2);

% normalization
F_norm = (s.frac+(1-s.frac).*(1-s.r))./s.z0;

% normalized correlation function
F = (s.frac.*(s.r+(1-s.r).*exp(-s.K.*tauGrid)).*exp(-kSqGrid.*s.D.*tauGrid.^s.alpha).*cxn_3d+(1-s.frac).*(1-s.r).*exp(-s.K.*tauGrid)./s.z0)./...
    F_norm;
