...
if ~is_vec
    amp_guess = interp2(tbi_sub_corr_avg,sub_mid_x,sub_mid_y,'spline');
else
    q_pt = max(sub_mid_x,sub_mid_y);
    amp_guess = interp1(tbi_sub_corr_avg,q_pt,'spline');
end
...
w0_guess = norm([sub_mid_y,sub_mid_x]-[min_dist_y,min_dist_x]); 
...
ub = [2*amp_bound,5*w0_guess,2*offset_bound];
