...
photophys_fluct_AC = s.k_off/s.k_on*exp(-K*tauGrid); % photophysical factor multiplying immobile correlation (fluctuation of state correlation)

photophys_norm = 1+s.k_off/s.k_on*exp(-K*tauNorm); % photophysical factor multiplying diffusing correlation in denom
photophys_fluct_norm = s.k_off/s.k_on*exp(-K*tauNorm); % photophysical factor multiplying immobile correlation in denom

Ik = exp(-s.w0^2/8*kSqGrid); % PSF in k-space
        
% normalization
F_norm = Ik.^2.*(s.frac.*photophys_norm.*exp(-kSqGrid.*s.diffusion*tauNorm)+(1-s.frac).*photophys_fluct_norm)+s.sigma;
